...
class ShowRepository @Inject constructor(
    ...
    override suspend fun getFavourites(): List<Show> = withContext(Dispatchers.IO) {
        showDao.getFavourites().map { it.toDomain() }
    }

    override suspend fun toggleFavourite(show: Show) = withContext(Dispatchers.IO) {
        if (show.isFavourite) {
    ...
    override suspend fun searchShows(query: String): List<Show> = withContext(Dispatchers.IO) {
        ...
        response.mapNotNull { searchResponse ->
            val show = searchResponse.show.toDomain()
            show.copy(isFavourite = show.id in favouriteIds)
        }
    }

    override suspend fun getShowById(id: Int): Show = withContext(Dispatchers.IO) {
